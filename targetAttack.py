...
class One_step_target(nn.Module):  # η = εsign (∇xJ(θ, x, y))
...
def testFGSM(model, device, val_dataset, val_loader, config, epsilon):
    ...
    model_state_file = 'output/OWN/crnn/2021-02-23-16-02/checkpoints/checkpoint_19_acc_0.9982.pth'
    if model_state_file == '':
    ...
    for data, target in val_loader:
        ...
        newLabels = getNewLabel(labels)

        # Call FGSM Attack
        perturbed_data = One_step_target(model, newLabels, converter, criterion, optimizer).attack(data, epsilon)
        ...
        #         correct += 1
        #     else:
        #         if len(adv_examples) < 3 and ('Z' in target):
        #             save(perturbed_data[i], data[i], pred)
        #             adv_ex = perturbed_data[i].squeeze().detach().cpu().numpy()
        #             adv_examples.append((target, pred, adv_ex))
        #
        #         # 预测标签包含'2'
        #         if ('Z' in target) and ('2' in pred):
        #             s += 1
    ...
    wrong_num = num_test_sample - correct
    probability = s / wrong_num
    print("Epsilon: {}\tTarget Probability = {} / {} = {:.4f}".format(epsilon, s, wrong_num, probability))

...
class PGD(nn.Module):  # Xt+1 = Πx+S (Xt + αsign (∇xL(θ, x, y)))
    ...
    def single_step_attack(self, x, mask):
        adv_x = x
        ...
        a = self.iter_eps * sign_data_grad
        if mask != None:
            a = a * mask
        adv_x = x + a
        return adv_x

    def pgd_attack(self, x, epsilon, mask=None):
        if self.rand_init:
        ...
        for i in range(self.nb_iter):
            next_adv_x = self.single_step_attack(adv_x, mask)
...
def testPGD(model, device, val_dataset, val_loader, config, epsilon):
    ...
    for data, target in val_loader:
        ...
        # Call PGD Attack
        perturbed_data = PGD(model, device, labels, converter, criterion, optimizer).pgd_attack(data, epsilon, mask)

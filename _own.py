...
class _OWN(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        newfilePath = os.path.join("pgdimage/q", img_name)

        img = Image.open(filePath)
        img.save(newfilePath, quality=20)
        img = cv2.imread(newfilePath)

        # img = cv2.imread(filePath)
        # img = compress_image(filePath, newfilePath)
...
def compress_image(filePath, newfilePath, mb=200, step=10, quality=80):
    """不改变图片尺寸压缩到指定大小
    :param infile: 压缩源文件
    ...
    while size > mb:
        ...
        if quality - step < 0:
            break
        quality -= step
        size = os.path.getsize(newfilePath)

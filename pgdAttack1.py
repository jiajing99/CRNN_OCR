...
class PGD(nn.Module):  # Xt+1 = Πx+S (Xt + αsign (∇xL(θ, x, y)))
    ...
    def pgd_attack(self, x, epsilon):
        if self.rand_init:
            x_tmp = x + torch.Tensor(np.random.uniform(-epsilon, epsilon, x.shape)).type_as(x).cuda()
        else:
            x_tmp = x
...
def testPGD(model, device, val_dataset, val_loader, config, epsilon, checkpointPath):
...
def main():
    ...
    eps = 0.3
    testPGD(model, device, val_dataset, val_loader, config, eps, checkpointPath)

# nohup python -u pgdAttack1.py > pgd1.log 2>&1 &

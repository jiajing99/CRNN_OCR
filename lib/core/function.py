...
def train(config, train_loader, dataset, converter, model, criterion, optimizer, device, epoch, writer_dict=None, output_dict=None):
    ...
    for i, (inp, idx) in enumerate(train_loader):
        ...
        if i%2 == 0:
            ...
            epsilon = 0.2
            perturbed_data = pgdAttack.usePGD(model, device, labels, converter, criterion, optimizer, inp, epsilon)
            # perturbed_data = fgsmAttack.useFGSM(model, labels, converter, criterion, optimizer, inp, epsilon)
            data = perturbed_data
        ...
        if i % config.PRINT_FREQ == 0:
            msg = 'Epoch: [{0}][{1}/{2}]\t' \
                  ...
                  'Loss {loss.val:.5f} ({loss.avg:.5f})\t'.format(
                      ...
                      speed=data.size(0)/batch_time.val,
